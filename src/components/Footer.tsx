...
import { RELEASE_STATUS } from "@/data/business";
  ...
  return (
    <footer className="ft mx-6 mt-10 mb-6 pt-4 border-t border-border text-[12px] text-muted flex flex-wrap gap-x-4 gap-y-1 justify-between">
      <span className="max-w-3xl">
        © 2026 TenAI · ContentCare — 마포구청 콘텐츠 위생관리. 시연용 표본 데이터 기준이며, 실제 수치는 첫 크롤 뒤 확정합니다. 원본은 수정하지 않고, 모든 처분은 사람이 승인합니다.
        <br />
        <span className="mono">{RELEASE_STATUS[0]}</span> <span className="mono">{RELEASE_STATUS[1]}</span>
      </span>
